...
- 生成 ENU $\rightarrow$ ECEF 变换矩阵 $
  M_{enu} = \begin {bmatrix}
...
* $x_{ecef} = M_{enu} \cdot x_{enu}$

- 原点（位于 ECEF 坐标系下 origin），X 轴（指向屏幕左），Y 轴（指向屏幕上），Z 轴（指向屏幕外）。
...
- position 变换为 invTransform 指定的坐标系，positionWC 不变
  - 第一次 updateMembers
